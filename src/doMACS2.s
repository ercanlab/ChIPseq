...
if [[ $kind == "single" ]]; then
  ...
  macs2 callpeak -t $chip -c $input --outdir MACSoutput -f BAM -g ce -n $name -q 0.05 --nomodel --extsize $fragment_size
else
  echo "${name} is merged: q = 0.01"
  macs2 callpeak -t $chip -c $input --outdir MACSoutput -f BAM -g ce -n $name -q 0.01 --nomodel --extsize $fragment_size
fi
