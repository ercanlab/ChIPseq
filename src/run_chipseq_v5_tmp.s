...
ercan_archive="/archive/s/se71/ercanlab"
...
egrep -A 5 "sample_id: [0-9]+" config_v1.yaml | while read -r line ; do
  val="$(cut -d ' ' -f 2 <<< $line)"
  if [[ -n "$(grep 'sample_id:' <<< $line)" ]]; then
    ...
    printf "${protein}" > $metadata_dir/protein_${sample_id}.txt
...
for sample_file in $metadata_dir/paired_*; do
  ...
  sample_id="${tmp%.txt}"
  read -r protein < $metadata_dir/protein_${sample_id}.txt
  printf "${chip_seqs} ${protein}\n" >> $WORKING_DIR/forUCSC.txt
  chip_seqs="${chip_seqs}_avg"
  inp_seqs="${inp_seqs}_avg"
  merged_chip="${chip_seqs}_chip_BOWTIE.bam"
  merged_inp="${inp_seqs}_input_BOWTIE.bam"
  printf "$merged_chip ${chips[*]}\n" >> $forMerge
  printf "$merged_inp ${inps[*]}\n" >> $forMerge
  printf "${chip_seqs}_chip ${chip_seqs} ${inp_seqs}_input ${inp_seqs}\n" >> $sample_file
  printf "$merged_chip $merged_inp ${merged_chip%%_BOWTIE.*} merged\n" >> $forMacs
done
...
# organize
mkdir -p InputSubtCoverage RatioCoverage RawInputCoverage
mv *SeqDepthNorm.bdg RawInputCoverage
mv *_inputsubt.bw InputSubtCoverage
mv *_ratio.bw RatioCoverage
mv InputSubtCoverage RatioCoverage RawInputCoverage MedianCoverage $WORKING_DIR
...
tail -q -n 1 $metadata_dir/paired_*.txt | awk '{print $2}' > names_merged.txt
...
# copy average bw file to ercan scratch
# cp *_*_inputsubt.bw /scratch/cgsb/ercan/bigwig/
